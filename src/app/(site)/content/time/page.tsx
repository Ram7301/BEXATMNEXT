...
const TimemanagementEdit: React.FC = () => {
  const router = useRouter();
  const [features, setFeatures] = useState<any[]>([]);
  const [image, setImage] = useState<File | null>(null);

  // ✅ Load Time Management features
  const loadFeatures = async () => {
    try {
      const res = await fetch("https://bexatm.com/ContentManageSys.php?contentId=T001");
      if (!res.ok) throw new Error("Failed to fetch time management features");
      const data = await res.json();
      setFeatures(data);
    } catch (err) {
      console.error("Error loading time management:", err);
    }
  };
  ...
  const saveFeatures = async () => {
    ...
    uploadImage();
    alert("✅ Time Management features updated!");
  };
              ...
              {features.map((feature, index) => (
                <div key={index} className="mb-8 border-b pb-6">
                  ...
                  <textarea
                    ...
                    onChange={(e) => handleChange(e, index)}
                    className="w-full mb-4 p-2 border rounded h-28"
                  />
                  ...
                  {/* Icon */}
                  <label className="block mb-2 text-sm font-medium">Icon (optional)</label>
                  <input
                    type="text"
                    name="icon"
                    value={feature.icon || ""}
...
export default TimemanagementEdit;
