...
const Managerdesk: React.FC = () => {
  ...
  const loadFeatures = async () => {
    try {
      const res = await fetch(
        "https://bexatm.com/ContentManageSys.php?contentId=M001"
      );
  ...
  const saveFeatures = async () => {
    await fetch("https://bexatm.com/ContentManageSys.php?contentId=M001", {
      method: "PUT",
        ...
        {features.map((feature, index) => (
          ...
          >
            ...
            <div className="lg:w-1/2">
              ...
              {/* Render icons below the description */}
              {feature.icon && Array.isArray(feature.icon) ? (
                <div className="flex gap-2.5 mt-4">
                  {feature.icon.map((icon: string, idx: number) => (
                    <Icon key={idx} icon={icon} className="text-2xl text-primary" />
                  ))}
                </div>
              ) : (
                feature.icon && (
